...
class TimeSeriesAnnotation(Plotter):
    """
    ...
    def __init__(self, x_axis:List | np.ndarray, list_of_data_or_plotter_object:List[np.ndarray | PlotterObject],
                 ...
                 save_callback: Callable[[np.ndarray], None],
                 displayed_window_size:int = 1_000,
                 one_time_jump: int = 300):
        super().__init__(x_axis, list_of_data_or_plotter_object, displayed_window_size, one_time_jump)
        self._x_clicked = []
    ...
    def repaint(self) -> None:
        ...
        super(TimeSeriesAnnotation, self).repaint()
